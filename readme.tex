...
\author{Jenny Huang}
\date{\today}
\title{CS480 Senior Design GUI Interface}
\hypersetup{
  pdfkeywords={},
  pdfsubject={},
...
I kept an original version of my \textbf{buttons$_{\text{bk}}$.h}, \textbf{buttons$_{\text{bk}}$.cpp}, and \textbf{main$_{\text{bk}}$.cpp} files in \textbf{./pic/backup$_{\text{problemSolving}}$/} folder for reference as they are the best indication of my problem-solving skills, because I didn't remove any trial that I have tried in order to get my project done.

\section{Course Introduction}
\label{sec-2}
\subsection{Reason for choosing this Course}
\label{sec-2-1}
This fall semester I didn't have many course to choose at all. The all choices for me possible includes: 
\begin{itemize}
\item Android App Programming
\item Fault Tolerance
\item Computer Networs Forensics
...
\subsubsection{Qt call: "try me\textasciitilde{}!"}
\label{sec-4-2-1}
From: me
...
\includegraphics[width=.9\linewidth]{./pic/Initial.png}

I states I worked two hours on the Qt, but actually I did work from 4:52pm \textasciitilde{} 8:06 pm three hours on the Qt, just that the first two hours are efficient, I used codes to implement the menubar. But the third hour was quite frustrating, because the wired Qt kept telling me layers on layers, blah\ldots{}blah\ldots{}blah\ldots{} 
...
\subsubsection{Snapshot Day performance}
\label{sec-4-2-3}
...
11/11 today will be the design review day. As deadline came closer and closer, my team-mates and I got to settle this GUI. On 11/7 afternoon, in the end of the team meeting, some team-mate asked if our interface is ready, and I answered my form interface hurts people's eyes. \textbf{Warning:} The form interface does hurt people's eyes, so be prepared\textasciitilde{}!

\includegraphics[width=.9\linewidth]{./pic/form.png}
...
\section{Software Engineer's Hurt}
\label{sec-6}
...
Form will always make interface ugly, I want to code the interface. "Tomorrow morning, I am going to code this gui, junk by junk, piece by piece, I must be able to do it\textasciitilde{}!" That evening, lying on the bed, I made up my mind that I must do it \textasciitilde{}~!!

\section{I can do it \textasciitilde{}~!}
\label{sec-7}

Next morning Friday morning at 8:00am, I started from the left hand side grid. It seems like projects hate people to make up their mind, cause once people has a mind, nothing can break it except human being themselves\textasciitilde{}!

I built left hand side grid in the morning, and I worked on searing directories images and adding buttons for "Grid Motion Menu" before, during and after Friday's TA meeting. I have worked about five hours (8\textasciitilde{}12\textasciitilde{}1-1class, 1-3pm I went home have rest) on Qt and up to 8:00pm before I went home, I was pretty much sure that I can do it \textasciitilde{}~!! 

At this point, I was very comfortable with c++ programming with more than 2 years programming experience. Then recall the GUI I designed last summer during the intern-ship, I had barely used Python before yet, it must be my mentor's great guide, team environment, together with my problem-solving skills, and passionate on projects that helped me to survive my very first GUI without understanding interface beforehand. How comfortable I was feeling right now with c++/Qt Creator programming, to the same extent how uncomfortable I was with Android App Programming course \textasciitilde{} I am not able to understand Android, still have minor Java programming uncomfortable, which resulted that I always depended on examples, and have barely any capability to develop my own code and project. How could I know what's the correct/good method to learn it?
...
\section{Problem-solving Skill Practise}
\label{sec-8}
...
\includegraphics[width=.9\linewidth]{./pic/IMG_0169.JPG}

\includegraphics[width=.9\linewidth]{./pic/IMG_0167.JPG}
...
\label{sec-11-3}
\begin{itemize}
\item Last color used could be preserved;
\item New picked color can be previewed;
\item Predefined 17 colors are clickable;
\item Design grid 72 buttons are clickable to get background color set;
\end{itemize}

\section{Easy Undone Parts for Fall semester}
\label{sec-12}
\begin{itemize}
\item Color Wheel was a plugin, and haven't link/activiate color-picking yet;
\item "Frame Start Time" \& "Increment Amout" needs to be in format of "00:00.000";
\item Latest color used is preserved, yet 2nd, 3rd, 4th Latest ones are not able to be remembered yet; Color queue 3 buttons behind direction square are not implemented (which is very easy).
\item RGB values are supposed to get value set from predefined color, color wheel, or latest 2-4 color queue, undone yet.
\item "Clear" \& 8 directions are easy to implement and will be done by end of this semester;
\item Yet I will take my time to finish all the color/time/direction related button slot functions by the end of this semester;
\end{itemize}
...
Since version \textbf{0.5}, it is an individual project, will \textbf{ONLY} update here since \textbf{11/20/2014}.
...
\subsection{Review Team Meeting on 11/16/2014 11:00-4:00 pm}
\label{sec-13-3}
\begin{itemize}
...
\subsection{version 0.6 11/21/2014, \textbf{Global Agreement - Graduate}, updates include}
\label{sec-13-6}
\begin{itemize}
\item Worked a little bit on the docs that I am required to do;
\item worked a little bit on the popup window when click/conduce "play";
\item Redesing the play command line buttons according to my taste;

\includegraphics[width=.9\linewidth]{./pic/2014-11-21_17:20:11_GlobalAgreementGraduate.png}
\end{itemize}

\subsection{\textbf{Global Agreement - Graduate}: 11/21/2014}
\label{sec-13-7}
\begin{itemize}
\item Reach \textbf{Global Agreement} with the department (my major advisor told me the good news) that I could \textbf{graduate} in summer 2015, with registering 1 credit part-time for coming spring semester;
\item There is \textbf{NO ANY} Ph.d on me, and my advisor clearly stated that nobody here in the department would want to block me here on campus forever.
\item Use picture ./pic/2014-11-21$_{\text{17}}$:20:11$_{\text{GlobalAgreementGraduate}}$.png to set a mark here;
\end{itemize}

\subsection{version 0.7 11/27/2014, updates include}
\label{sec-13-8}
\begin{itemize}
\item Highlight valid frame \& current viewing frame bolder to be \textbf{Red} color;
\item Finished a simple \textbf{Animation Popup} window design and implementation as layout below. This popup window should be linked with "\textbf{play}" button when it gets clicked. Popup window is designed as to the client's requirement that he doesn't want to get disrupted by the other neighboured frames in main window. So the popup window frame is not highlighted to be RED any more. I could also remove the frame border if the client want that layout.

\includegraphics[width=.9\linewidth]{./pic/Screenshot_from_2014-11-27_23:34:06.png}
\item Redesing \& Implemented left handside working space QGridLayout to be 20*12 row*col, instead of old one 12*6 row*col;
\item Redesing \& Implemented bottom right predefined colors to be user custom colors with 34*2 options interactively changable/redefinable by user;
\item Redesign \& Implemented right bottom working area so it has a better layout;
\item Redesign \& Implemented QGridLayout setSpacing() values so that the working space grids and preview layouts mimic that actual tower window size;
\item Current main window layout looks at listed below:

\includegraphics[width=.9\linewidth]{./pic/Screenshot_from_2014-11-27_22:42:34.png}
\end{itemize}

\subsection{version 0.8 12/05/2014, updates include}
\label{sec-13-9}
will try to update 12/05/2014 as one version tomorrow
\subsubsection{Project Shortcomings}
\label{sec-13-9-1}
\begin{itemize}
\item \textbf{Shortcomings} of this project implementation include: memory leak problems. I have created too many pointer to objects, but in my destructor, I was not aware the \textbf{delete} step for the \textbf{new} operator objects that I have created. 
\begin{itemize}
\item \textbf{Solution 1:} I should delete all the new operator created pointer in my \textbf{\textasciitilde{}MainWindow} destructor in order to release resources. Realized and listed this shortcoming here, I will take my time later on when I finished my priorities to. Add all the \textbf{new} operator pointers to be MainWindow private pointers; Add the \textbf{delete} operations in MainWindow destructor for all the new operator pointers;
\item \textbf{Solution 2:} Instead of using \textbf{new} operator to create them, maybe I could create them on stack;
\item \textbf{Solution 3:} Qt specific solution. Link the layouts to their parent so that when close, everything will be handled by Qt smartly. I cleaned some unnecessary widget pointer declaration in the \textbf{mainwindow.h} file which were mainly for menubar; I have done about first 300 lines of codes linking in my \textbf{mainwindow.cpp} file, but the labor for the rest parts can be done later instead of now.
\end{itemize}
\item \textbf{Shortcomings} of this project implementation include: I still remembered the very convenient and readable camelCase naming convention my internship mentor applied on his Python Test Suite Automation Framework, like "\_\_" prefix for variables wihtin a class means the variables are private class variables, together with those camelCase variable and function names. But for this Senior Design project, when I implemented the GUI, it was my fault that I completely left this naming convention behind forgotten. Here are some points to address this (\textbf{Quote} from <Programming with Qt: Writing Portable GUI Applications on Unix and Win32> page 422 by \textbf{Mattias Dalheimer}). 
\begin{itemize}
\item In a large project, you man have many dialog box (or other) classes generated by Qt Designer and uic (Qt Creator too?) and implementation classes for many of those dialog boxes. In such a situation, it is best to think of a smart naming scheme right away to avoid confusion later.
\item There are two demands on such a naming scheme: it should be easy, obvious, and unique to get from the generated class name to the implementation class name (or the other way around), and the class name should not be too cumbersome to type.
\item One naming scheme that fulfills these conditions is that the generated dialog box class gets the name that seems "natural" (such as \textbf{PizzaEntry}), and the implementation class gets the same name with \textbf{Impl} attended.
\item Another option that some people prefer is to give the generated class a name that ends in \textbf{Base}, such as \textbf{PizzaEntry*Base}, and the subclass the original name, such as \textbf{PizzaEntry}. Since you willl use the subclass only in your code, naming the class in this way can keep your code a bit more readable.
\item Many schemes are equally good. The important thing is that you pick one consistent scheme and stick to it throughout your project.
\end{itemize}
\end{itemize}
\subsubsection{Design \& Implementation Updates}
\label{sec-13-9-2}
\begin{enumerate}
\item General Updates
\label{sec-13-9-2-1}
\begin{itemize}
\item Redesing \& Implemented right bottom working area so it includes the \textbf{priority options} layout on the right which sets priorities when different patterns merge;
\item Highlight \textbf{Red:} \textbf{Green:} \textbf{Blue:} texts to be the specific color as well so that it's a more obvious indication layout;
\item Design \& Implemented \textbf{Frame Start time:} QDoubleSpinBox to show 8-digit hh:mm:ss timer counter; Design \& Implemented \textbf{Increment Amout:} QDoubleSpinBox to show 9-digit mm:ss.001sec timer counter as well, by inherit QDoubleSpinBox to override a member function; I have tried and finished an python example layout stored in \textbf{./pic/backup$_{\text{problemSolving}}$/} folder, \textbf{QxDoubleSpinBox.py} and \textbf{Window.py} refered to a website listed below, can be refered as problem-solving indication. \textbf{Note:} Signals and Slots are working fine here, but minor issues with seconds showing and milloseconds showing for frame and Increment respectively; having not got any suggestive idears yet, but will look into it later on;
\item Added .wav audio file parser into GUI for timestamps; Suggestive layout comes out to been listed below, which has a timestamp double spinbox to show the detailed time and parsed wave drawing picture: 

\includegraphics[width=.9\linewidth]{./pic/Screenshot_from_2014-12-05_07:48:18.png}
\item My design for user predified colors, Qt Quick dynamic JS creation or regular QPushButton one as I am doing now? Maybe I should implement the project piece by piece, from easy to difficult updated implementation;
\item Same happens to .wav file parser and graphics generatation. It's an interesting project with many potentials. I will come back to work on this one later when I solved my \textbf{priorities}, like my first priority will be to find a job which \textbf{sponsors H1B working Visa} for me before \textbf{4/1/2015}.
\item worked on the documents a little bit, the file can be view from \textbf{./mydocs/seniorDesign.org} file; From very beginning I never liked to write the document, but still even after I have put hard work on it, I still needs to finish the software design document according to requirement even individually.
\item Snapshot \#2 will be from 8:30-10:20, but the final document report is not required for today.
\item will add \textbf{Increment Amout} to \textbf{menu} option as well tomorrow;
\end{itemize}
\item Signals \& Slots (Qt Essential)
\label{sec-13-9-2-2}
\begin{itemize}
\item Linked \textbf{ColorWheel} signal with MainWindow slot function to get full color-picker function from this color-wheel, \textbf{done} with \textbf{color-wheel} functions, but the implementation ideas for color-wheel should be learned and grasped and adapted to be my own ideas for future implementations;
\item for Qt Signals and Slots, so far I got three ideas how to implement the connection between signals and slots;
\begin{itemize}
...
\end{itemize}
\end{itemize}
\item Qt: Code Less; Create More; Deploy Anywhere.
\label{sec-13-9-2-3}
current snampshot:

\includegraphics[width=.9\linewidth]{./pic/Screenshot_from_2014-12-04_18:22:35.png}
\end{enumerate}
\subsubsection{docs}
\label{sec-13-9-3}
\begin{itemize}
\item Worked a little bit on the docs;
\end{itemize}
\subsection{version 0.9 12/18/2014, updates include}
\label{sec-13-10}
\begin{itemize}
\item I got version 2 feedback from the course instructor on Tuesday (12/16/2014) noon at around 1:00pm, and deadline set to be Thursday (12/18/2014) 5:00pm;
\item I updated the docs report according to the instructor's marks and turned it in a moment ago.
\item Make only minor changes to the Loc class function names;
\item By prints strings out, connected signlas and slots for all left side working area, and decorated with some patterns, like heart, simple ware, and "T" and another shape circling with 90 degree change, just for fun;
\item This will be the last update for this repository, I won't come back to this repository until I feel confident with my algorithms review for my job interviews, which will be at least two months later.
\item But I will make my own effect to get key issues of this project done, like signal-slots, keyevents, mouseevents, so that I learn.
\item I had to say I have wasted too much time on this project already, but I did have fun, and I will continu to learn how to solve problems.
\item A current snapshot is attached followed:
\end{itemize}

\includegraphics[width=.9\linewidth]{./pic/Screenshot_from_2014-12-18_13:48:44.png}
...
\item Basic layouts
\begin{itemize}
\item \url{http://qt-project.org/doc/qt-4.8/layouts-basiclayouts.html}-
\end{itemize}
\item layout example
\begin{itemize}
\item \url{http://zetcode.com/gui/qt4/layoutmanagement/}
\end{itemize}
\item my$_{\text{grid}}$$_{\text{layout}}$
\begin{itemize}
\item \url{http://stackoverflow.com/questions/9660080/how-does-one-fill-a-qgridlayout-from-top-left-to-right}
\item \url{http://stackoverflow.com/questions/8685005/qt-making-a-splitter-horizontal-and-vertical-at-same-time}
\end{itemize}
\item Layout management: find file, directories and subdirectories
\begin{itemize}
\item \url{http://www.informit.com/articles/article.aspx?p=1405543}
\end{itemize}
\item subclass for buttons
\begin{itemize}
\item \url{http://qt-articles.blogspot.com/2010/06/how-to-customize-button-in-qt.html}
\item \url{http://stackoverflow.com/questions/3137805/how-to-set-image-on-qpushbutton}
\end{itemize}
\item float spin box
\begin{itemize}
\item \url{http://www.qtforum.org/article/12505/qspinbox-with-float-values.html?s=b97a1086f89c740edca22c410bdfec398bb41f5d}
\end{itemize}
\item QGraphicsView
\begin{itemize}
\item \url{http://stackoverflow.com/questions/1357960/qt-jpg-image-display}
\end{itemize}
\item RGB Color
\begin{itemize}
\item \url{http://help.dottoro.com/labpxqqe.php}
\item \url{http://www.rapidtables.com/web/color/RGB_Color.htm}
\item \url{http://cloford.com/resources/colours/500col.htm}
\end{itemize}
\item QImageButton
\begin{itemize}
\item \url{https://github.com/liuyanghejerry/Qt-Plus/blob/master/develop/QImageButton/qimagebutton.h}
\item \url{https://github.com/liuyanghejerry/Qt-Plus/blob/master/develop/QImageButton/qimagebutton.cpp}
\end{itemize}
\item Naming convention
\begin{itemize}
\item \url{http://books.google.com/books?id=Nm6cAgAAQBAJ&pg=PA422&lpg=PA422&dq=Qt+subclass+does+not+name+a+type&source=bl&ots=GTmBrz-wSG&sig=7T5KsdopRC4rbU63A9XeP7VxXzg&hl=en&sa=X&ei=QDl6VIaeKsfksASf5oDYAQ&ved=0CD4Q6AEwBTgK#v=onepage&q=Qt\%20subclass\%20does\%20not\%20name\%20a\%20type&f=false}
\end{itemize}
\item Signal Slot
\begin{itemize}
\item \url{http://blog.csdn.net/m6830098/article/details/13058459}
\item \url{http://www.ibm.com/developerworks/cn/linux/guitoolkit/qt/signal-slot/}
\item \url{http://blog.csdn.net/qter_wd007/article/details/5598264}
\item \url{http://doc.qt.digia.com/4.6/signalsandslots.html}
\item override slot \url{http://www.cnblogs.com/rickyk/p/3792149.html}
\item Inheritance: \url{http://xhrwang.me/2014/07/13/qt-signals-and-slots.html}
\item \url{http://wenku.baidu.com/view/f802b33ea32d7375a417802e}
\item \url{http://blog.sina.com.cn/s/blog_63913ba6010133r9.html}
\item \url{http://www.codeproject.com/Articles/21909/Introduction-to-dynamic-two-dimensional-arrays-in}
\item \url{http://www.tutorialspoint.com/cplusplus/cpp_overloading.htm}
\item \url{http://www.tutorialspoint.com/cplusplus/subscripting_operator_overloading.htm}
\item \url{http://www.qtcentre.org/threads/1345-Multiple-Inheritance-amp-Qt}
\end{itemize}
\item Overload functions casts
\begin{itemize}
\item \url{http://qt-project.org/forums/viewthread/23846}
\end{itemize}
\item Signal Slot with threads
\begin{itemize}
\item \url{http://www.360doc.com/content/12/1226/12/9200790_256340719.shtml}
\end{itemize}
\item QLabel text color set
\begin{itemize}
\item \url{http://stackoverflow.com/questions/2749798/qlabel-set-color-of-text-and-background}
\end{itemize}
\item QPushButton text Alignment
\begin{itemize}
\item \url{http://www.qtcentre.org/threads/24872-QPushButton-setText-Alignment}
\end{itemize}
\item QDoubleSpinBox 
\begin{itemize}
\item python subclass startpoint: \url{http://osdir.com/ml/python-pyqt-pykde/2009-10/msg00103.html}
\item sub QDoubleSpinBox QAbstractSpinBox: \url{http://stackoverflow.com/questions/26537473/qdoublespinbox-with-leading-zeros-always-4-digits}
\item \url{http://www.qtada.com/documentation/qtada-3.0.0/qtada_ugr/subclassing-qobject.html}
\end{itemize}
\item ScrollArea paint
\begin{itemize}
\item \url{http://wolfand11.sourceforge.net/?p=2250}
\end{itemize}
\item QPtrList 
\begin{itemize}
\item \url{http://mobile.51cto.com/symbian-270583.htm}
\end{itemize}
\item Qt focus Policy
\begin{itemize}
\item \url{http://mobile.51cto.com/symbian-270605.htm}
\end{itemize}
\item Qt Quick colorPicker JS dynamic
\begin{itemize}
\item \url{http://blog.csdn.net/foruok/article/details/32730911}
\end{itemize}
\item Qt style sheet 
\begin{itemize}
\item \url{http://www.cnblogs.com/csuftzzk/p/qss_lineedit_completer.html}
\end{itemize}
\item private classes and D-pointers
\begin{itemize}
\item \url{http://zchydem.enume.net/2010/01/19/qt-howto-private-classes-and-d-pointers/}
\end{itemize}
\item the event system
\begin{itemize}
\item \url{http://qt-project.org/doc/qt-5/eventsandfilters.html}
\end{itemize}
\item Menu separator
\begin{itemize}
\item \url{http://qt-project.org/doc/qt-4.8/mainwindows-menus.html}
\end{itemize}
\item Menu submenu
\begin{itemize}
\item \url{http://stackoverflow.com/questions/13799033/how-to-add-submenu-in-qt}
\end{itemize}
\item EventFilter
\begin{itemize}
\item \url{http://blog.csdn.net/zhoxier/article/details/7655775}
\end{itemize}
\item EventHandler
\begin{itemize}
\item \url{http://www.360doc.com/content/12/0809/15/1291795_229229792.shtml}
\end{itemize}
\item c++ overload operator
\begin{itemize}
\item \url{http://en.cppreference.com/w/cpp/language/operators}
\end{itemize}
\item Shapes
\begin{itemize}
\item \url{http://qt-project.org/doc/qt-4.8/graphicsview-diagramscene.html}
\end{itemize}
\end{itemize}
